...
public interface Configuration extends ConfigurationNodeSource {
	...
	/**
	 * Adds the given node to the configuration structure This will attempt to use the node's existing parents in
	 * the configuration structure where possible
	 *
	...
	/**
	 * The path separator to use with {@link #getNode(String)} The path separator splits paths as a literal string,
	...
	/**
	 * Sets this configuration's path separator. More information on how the path separator functions in
	...
	/**
	 * Whether this configuration writes default values (from {@link ConfigurationNode#getValue(Object)} to the
	 ...
	 * Normally this just splits the path with the {@link #getPathSeparator()}, but can limit how deep a child path
	 * can go or whether this configuration can even have children.
	 *
	...
	/**
	 * Make sure the provided path meets the requirements. A correct implementation of Configuration will impose the
	 * same restrictions on this and {@link #splitNodePath(String)}, so invoking this method on an array from
	 * {@link #splitNodePath(String)} would return the original array.
	 *
